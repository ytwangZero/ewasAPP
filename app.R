...
server <- function(input, output, session) {
  ...
  datasetInput <- eventReactive(input$submit,{
    ...
    chunk.size <- ceiling(len/input$core)
    if(input$model %in% c("lm","lmer")){
      if(input$expovar  %in% facvars){
        system.time(
          modelres <- foreach(i=1:input$core, .combine='rbind') %dopar%
            {  # local data for results
              res <- matrix(0, nrow=min(chunk.size, len-(i-1)*chunk.size), ncol=3*(facnum-1))
        ...
        system.time(
          modelres <- foreach(i=1:input$core, .combine='rbind') %dopar%
            {  # local data for results
              res <- matrix(0, nrow=min(chunk.size, len-(i-1)*chunk.size), ncol=3)
              for(x in ((i-1)*chunk.size+1):min(i*chunk.size, len)) {
                res[x - (i-1)*chunk.size,] <- as.numeric(base::t(ewasfun(methydata[x,],formula,covdata)))
              }
              # return local results
      ...
      system.time(
        modelres <- foreach(i=1:input$core, .combine='rbind') %dopar%
          {  # local data for results
            res <- matrix(0, nrow=min(chunk.size, len-(i-1)*chunk.size), ncol=4)
            for(x in ((i-1)*chunk.size+1):min(i*chunk.size, len)) {
              res[x - (i-1)*chunk.size,] <- as.numeric(base::t(ewasfun(methydata[x,],formula,covdata)))
            }
            # return local results
